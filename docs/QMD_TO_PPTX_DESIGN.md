...
DOMツリーのルートから深さ優先でノードを走査する。ノードの種別を上記の判定方法で識別し、ノード種別とノード内容を `DOMNodeInfo` のリストとして返す。スライドレンダラーはそのリストをイテレートし、各 `DOMNodeInfo` の種別に応じて対応するレンダラー（テキスト・Mermaid・数式）を呼び出す。
...
Mermaidレンダラーは `qmd_to_pptx.mermaid` サブパッケージに分割されている。`MermaidRenderer` がファサードとして機能し、ダイアグラム種別に応じて専用レンダラーへ処理を委譲する。

```mermaid
flowchart TD
    A[MermaidRenderer\nファサード] --> B{graph_type}
    B -- flowchart/graph --> C[FlowchartRenderer]
    B -- class --> D[ClassDiagramRenderer]
    B -- stateDiagram --> E[StateDiagramRenderer]
    B -- er --> F[ErDiagramRenderer]
    B -- mindmap --> G[MindmapRenderer]
    C & D & E & F & G --> H[BaseDiagramRenderer\n共通ユーティリティ]
```

**FlowchartRenderer の機能：**

| 機能 | 内容 |
|---|---|
| ノード形状 | 14種類（矩形・丸角・スタジアム・サブルーチン・シリンダー・円・非対称・ひし形・六角形・右傾斜・左傾斜・台形・逆台形・二重円） |
| エッジ矢印 | 7種類（開放線・通常矢印・両方向・丸端・両方向丸端・×端・両方向×端） |
| 線種 | 4種類（実線・破線・太線・不可視）。不可視はコネクターを生成しない |
| エッジラベル | エッジテキストが設定されている場合、コネクター中点にテキストボックスを追加する |

**処理詳細（フローチャート系）：**

1. mermaid-parser-pyを用いてMermaidテキストからノード（vertices）とエッジ（edges）の情報を抽出する
2. 抽出した情報を基にNetworkXのグラフオブジェクトを構築する
3. NetworkXの`spring_layout`アルゴリズムを適用して各ノードの2次元座標を計算する
4. vertices の `type` フィールドに基づき `_SHAPE_MAP` からAutoShapeTypeを選択して `add_shape()` でスライドに配置する
5. edges の `stroke` フィールドで線種、`type` フィールドで矢印種別を設定した `add_connector()` でノード間を接続する
6. `text` フィールドが空でない場合はコネクター中点座標にテキストボックスを追加する

**処理詳細（クラス図）：**

1. classes 辞書から各クラスをヘッダー・属性・メソッドの3段構成矩形としてグループ化して描画する
2. relations リストの `relationType` に応じて継承（三角矢印）・コンポジション（大ひし形）・集約（中ひし形）・依存（開放矢印）のUML矢印スタイルでコネクターを描画する
3. `lineType=1` の場合は点線コネクターとして描画する
