...
class FlowchartRenderer(BaseDiagramRenderer):
    ...
    def _group_node_with_labels(
        ...
        slide : Slide
            python-pptxのSlideオブジェクト。
        node_shape : object
            グループ化対象の始点ノードShapeオブジェクト。
        txboxes : list
            グループ化対象のラベルテキストボックスShapeオブジェクトのリスト。
        """
        ...
        # spTree からノードシェイプとテキストボックスを取り出して grpSp に移動する
    ...
    def _add_edge_label_near_source(
        ...
        """
        エッジラベルを始点ノードの接続点近くに独立テキストボックスとして配置する。

        始点から終点方向へノード幅の0.6倍ほどオフセットした位置に、
        背景・枠線なしのテキストボックスを配置する。
        コネクター自体にテキストを埋め込む方式（中央固定）と異なり、
        始点側の接続点近くにラベルを表示できる。
        ...
        fill = txBox.fill
        fill.background()
        ln_el = txBox._element.find(qn("p:spPr"))
        if ln_el is not None:
            existing_ln = ln_el.find(qn("a:ln"))
            if existing_ln is None:
                existing_ln = lxml_etree.SubElement(ln_el, qn("a:ln"))
            # 線なし設定
